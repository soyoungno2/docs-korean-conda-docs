...
.. raw:: html
   ...
   <H2>설명</H2><PRE>
          ...
          이 명령은 패키지 이름의 목록을 받아 환경의 모든 패키지와 호환되는 최신 버전으로 업데이트한다.

          콘다는 요청된 패키지를 최신 버전으로 업데이트하려 한다.
          이를 위해 이미 설치된 패키지를 업데이트 하거나 추가 패키지를 설치할 수 있다.
          기존 패키지 업데이트를 막고 싶다면 <B>--no-update-deps</B> 옵션을 사용한다.
