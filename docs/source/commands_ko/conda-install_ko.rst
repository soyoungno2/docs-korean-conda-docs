...
.. raw:: html
   ...
   <H2>설명</H2><PRE>
          ...
          이 명령은 패키지 사양 목록을 받고 그 사양과 동일하면서 밑단의 환경과 호환되는 패키지의 모음을 설치한다.
          ...
          이 옵션을 사용하면 강제로 요청된 패키지의 구버전을 설치해 추가 의존 패키지 설치를 막지 않을 수도 있다.

          의존 요소 확인를 모두 건너뛰고 싶다면 '--force' 옵션을 사용한다.
          이 옵션으로 환경과 호환되지 않는 패키지가 설치될 수 있기 때문에 주의해서 사용한다.

          콘다는 명시적인 콘다 패키지 파일명 목록으로 호출될 수도 있다. (e.g.  ./lxml-3.2.0-py27_0.tar.bz2).
          이러한 방식의 콘다 사용은 <B>--force</B> 옵션을 내포하기 때문에 위와 같은 이유로 주의해야 한다.
