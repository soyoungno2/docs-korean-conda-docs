...
.. raw:: html
   ...
   <H2>설명</H2><PRE>
          사용법: conda install [-h] [--revision REVISION] [-y] [--dry-run] [-f]
          ...
          패키지 목록에 있는 패키지를 지정된 콘다 환경에 설치한다.

          이 명령은 패키지 사양 목록을 받고 그 사양과 동일하면서
          밑단의 환경과 호환되는 패키지의 모음을 설치한다.
          호환성을 완전히 만족할 수 없으면 에러가 보고되고 환경은 변경되지 않는다.

          콘다는 요청된 패키지의 가장 최신 버전을 설치하려 한다.
          이를 위해 이미 설치된 패키지를 업데이트하거나 추가 패키지를 설치할 수 있다.
          기존 패키지의 업데이트를 막고 싶다면 <B>--no-update-deps</B> 옵션을 사용한다.
          이 옵션을 사용하면 강제로 요청된 패키지의 구버전을 설치해
          추가 의존 패키지 설치를 막지 않을 수도 있다.

          의존 요소 확인를 모두 건너뛰고 싶다면 '--force' 옵션을 사용한다.
          이 옵션으로 환경과 호환되지 않는 패키지가 설치될 수 있기 때문에 주의해서 사용한다.

          콘다는 명시적인 콘다 패키지 파일명 목록으로 호출될 수도 있다.
          (예시:  ./lxml-3.2.0-py27_0.tar.bz2).
          이러한 방식의 콘다 사용은 <B>--force</B> 옵션을 내포하기 때문에 주의한다.
          명시적인 파일명과 패키지 사양은 한 명령에서 함께 사용될 수 없다.
   ...
   <H2>옵션</H2><PRE>
      ...
      <B>선택</B> <B>인수:</B>
          ...
          <B>-p</B> PATH, <B>--prefix</B> PATH
                 콘다 환경 접두어로의 전체 경로다.
                 (기본: <I>root</I> <I>prefix/envs/conda-docs</I>).
